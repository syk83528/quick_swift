...
public class ActivityIndicator: UIView, CAAnimationDelegate {
    ...
    @IBInspectable public var fontSize: CGFloat = 18 {
        didSet {
            progressLabel.font = .medium(fontSize)
        }
    }
    ...
    override public func layoutSubviews() {
        ...
        progressLabel.center = CGPoint(x: self.bounds.midX, y: self.bounds.midY - 1)
    }
    ...
    public enum CompletionType: Int {
        case success, failed, alert
    }
    
    public func completeLoading(_ completionType: CompletionType, completion: Block? = nil) {
        ...
        case .alert:
            setStrokeAlertShapePath()
            strokeStart = 0.12
            strokeEnd = 0.6
            
            phase1Duration = 0.5 * dhCompletionAnimationDuration
            phase2Duration = 0.5 * dhCompletionAnimationDuration
        }
        ...
        tailEndAnimation.toValue = (completionType == .success || completionType == .alert) ? 1.0 : strokeEnd
    ...
    private func setProgressLayerPath() {
        ...
        let radius = (min(self.bounds.width, self.bounds.height) - progressLayer.lineWidth) / 2.5
    ...
    private func setStrokeAlertShapePath() {
        let width = self.bounds.width
        let height = self.bounds.height
        let square = min(width, height)
        let b = square/2
        let space = square/2
        let point = CGPoint(x: b, y: square / 4)

        let path = CGMutablePath()
        path.move(to: point.offset(x: space / 12, y: -(space / 8)))
        let bottomLeft = CGPoint(x: point.x - space / 1.5, y: point.y + space)
        path.addLine(to: bottomLeft)
        let bottomRight = CGPoint(x: point.x + space / 1.5, y: point.y + space)
        path.addLine(to: bottomRight)
        path.addLine(to: point)
        
        let dot = point.offset(y: space / 2.5)
        path.move(to: dot)
        path.addLine(to: dot.offset(x: 0, y: 0.5))
        
        let dash = dot.offset(y: space / 5.5)
        path.move(to: dash)
        path.addLine(to: dash.offset(x: 0, y: space / 4.5))
    ...
    private func alertJoinPoint() -> CGPoint {
        let r = min(self.bounds.width, self.bounds.height)/2
        let k = lineWidth/2
